...
def main():
    ...
    Amaç: MLlib'in temel taşı olan 'Pipeline' (Boru Hattı) kavramını göstermektir:
     - veri yükleme
     - training ve test verisini bölme
     - boru hattı (pipeline) oluşturma
    ...
    spark = SparkSession.builder.appName("MLlibPipelineDemo").getOrCreate()
    ...
    # 3. BORU HATTI (Pipeline) TANIMLAMA
    # Makine öğrenimi iş akışının tamamını bir 'Pipeline' olarak tanımlarız.
    # Bu, "Feature Engineering" (Özellik Mühendisliği) adımlarını ve Modeli uygun şekilde zincirler
    # ve kullanıcı için daha kolay hale getirir.
    print("🔧 Pipeline (Boru Hattı) tanımlanıyor...")
    ...
    # Adım 3d-3e: Tüm adımları zincirle
    # Bu adımların sırayla uygulanacağını Spark'a söylüyoruz:
    pipeline = Pipeline(stages=[label_indexer, assembler, dt])

    # 4. MODEL EĞİTİMİ (TRAIN)
    print("🧠 Model (Pipeline) eğitiliyor...")
    model = pipeline.fit(training_data_raw)
    print("✅ Model (Pipeline) eğitildi.")

    # 5. TAHMİN VE DEĞERLENDİRME (EVALUATE)
    # Eğitilmiş 'model'i (pipeline'ı) test verisi üzerinde uyguluyoruz.
    # Spark, 3 adımı da (indexer, assembler, tree) sırayla test verisine uygular.
    print("🧪 Test verisi üzerinde tahmin yapılıyor... (Bu bir 'Dönüşüm'dür)")
    predictions = model.transform(test_data_raw)
